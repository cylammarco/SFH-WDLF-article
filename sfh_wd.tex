...
of stars, including white dwarfs, main sequence stars, sub-giants and the use
of the entire stellar population available to their data. We find convincing
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Most WDs have a similar broadband colour to main sequence stars, they cannot be
...
work, we use lower case italicized $m$ for apparent magnitude, uppder case
...
work. $t$ is for time and $\tau$ is the inverse cooling rate.
...
happened at a look back time of 0.3\,Gyr. After a long hiatus, with new
...
deconvolution method~\citep{1972JOSA...62...55R, 1974AJ.....79..745L} that are 
...
enhanced star formation~(T14). This is due to the large number of degenerate
solutions that can yield the WDLF agree to within the uncertainty. Some forms
...
the large possible number of degenerate solutions and noise can significantly
alter the solution, regularization is necessary to avoid erroneously amplified
...
Section~\ref{sec:application}), it is obvious that I08's statement only holds
if the WDLFs do not extend into magnitudes fainter than $\approx14.5$\,mag.
Neither does it apply to short bursts of star formations. And we shall see in
the next section that the peak magnitude of a pWDLF is the determining feature.
...
method. The free parameters to be fitted are the weights to sum the pWDLF to
give the modelled WDLF for comparing against the observed WDLF. The weight to
...
In this work, we fit with 45 pWDLFs~(wee Section~\ref{sec:application}). The
...
This set of WDLFs demonstrates that the method can retrieve both sharp and
...
magnitude binning~(see Figure~\ref{fig:nr_sfr}). The corresponding
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
C23 made use of a similar sample to this work, they found a constant
...
telescope mirror~(or even to model the Earth's atmosphere if adaptive optics is
used) to the detector plane. Very similar reasoning is used in, and not limited
...
using WDLF with their uncertainties that encompass the uncertainties in the WD
...
\begin{enumerate}
    ...
    \item The properties of the Galaxy are assumed to be static, for example,
    ...
    should be investigated, the shape of the SFHs from different works, roughly
    agrees to when the most recent strong star formation happened. However,
    with the large distance \textit{Gaia} probing (for the relatively bright
    WDs), the line-of-sight can go through multiple stellar populations.
\end{enumerate}
